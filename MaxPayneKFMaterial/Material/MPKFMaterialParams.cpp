...
void MPKFMaterialPBAccessor::Set(PB2Value& val, ReferenceMaker* owner, ParamID id, int tabIndex, TimeValue t)
{
	...
	case mpkfmaterial_params_has_lit:
		...
		break;
	case mpkfmaterial_params_has_specular:
		Material->HasSpecularTexture = val.i;
		break;
...
static ParamBlockDesc2 mmpkftexture_param_blk(mpkfmaterial_params, _T("params"), 0, GetMPKFMaterialDesc(),
	...
	mpkfmaterial_params_specular, _T("specular_texmap"), TYPE_TEXMAP, P_OWNERS_REF | P_SHORT_LABELS, IDS_KF_MATERIAL_SPECULAR_TEXMAP,
	p_refno, MPFKMaterial_SPECULAR_TEXMAP_REF,
	p_subtexno, MPFKMaterial_SPECULAR_TEXMAP,
	...
	p_accessor, &gMPKFMaterialAccessor,
	p_default, FALSE,
	p_ui, mpkfmaterial_params_textures_rollout, TYPE_SINGLECHECKBOX, IDC_KF_MATERIAL_SPECULAR_CHECKBOX,
	p_end,
...
MPKFMaterialBasicDlgProc::MPKFMaterialBasicDlgProc(MPKFMaterial* InMaterial, IMtlParams* InIp):
	Handle{ nullptr },
	Material{ InMaterial },
	Ip{ InIp }
{
...
MPKFMaterialBasicDlgProc::~MPKFMaterialBasicDlgProc()
{
	DLSetWindowLongPtr(Handle, NULL);
	Handle = nullptr;
}

void MPKFMaterialBasicDlgProc::UpdateMtl()
{
	if (Material->TestMtlFlag(MTL_TEX_DISPLAY_ENABLED) && Ip) {
		Ip->MtlChanged();
	}
...
INT_PTR MPKFMaterialBasicDlgProc::DlgProc(TimeValue t, IParamMap2* map, HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	int ID = LOWORD(wParam);
	int Code = HIWORD(wParam);
	...
	switch (msg) {
		...
		case WM_COMMAND: {
			switch (ID) {
				case IDC_KF_MATERIAL_COLOR_DIFFUSE:
				case IDC_KF_MATERIAL_INVISIBLE_GEOMETRY:
				case IDC_KF_MATERIAL_TWO_SIDED:
				case IDS_KF_MATERIAL_VERTEX_ALPHA: {
					UpdateMtl();
					return TRUE;
				}
			}
