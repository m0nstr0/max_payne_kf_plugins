...
public:
	...
	virtual bool Connect(ReferenceTarget* fromParent, int fromSubNum,
							...
							const MCHAR* toExpr)=0;		 // set up a one-way wire from -> to
	...
	virtual bool Connect2Way(ReferenceTarget* leftParent, int leftSubNum,
							...
							const MCHAR* leftExpr, const MCHAR* rightExpr=NULL)=0;     // set up a two-way wire
	...
	\param iFlags - one of the <a href="group___param_wire_flags.html">parameter wire flags</a>.  This sets which which action to take.
...
class IBaseWireControl : public StdControl, public FPMixinInterface {
	public:
		...
		virtual const MCHAR*	get_expr_text(int i)=0;
		...
		virtual void	set_expr_text(int i, const MCHAR* text)=0;

		// animation sub-controller
		/*! \remarks This method allows you to set the driven animation
		controller.
		\param c - A pointer to the controller you wish to set. */
		virtual void	set_driven_animation(Control* c)=0;
		/*! \remarks This method returns a pointer to the driven animation
		controller. */
		virtual Control* get_driven_animation()=0;

		// type predicate
		/*! \remarks This method will return TRUE if the wire is a driver
		predicate, otherwise it will return FALSE. */
		virtual bool    is_driver()=0;
		/*! \remarks This method will return TRUE if the wire is a driven
		predicate, otherwise it will return FALSE. */
		...
		/*! \remarks This method validates consistency of the WireController's linkages. */
		virtual bool doValidate()=0;

		/*! \remarks This method fixes inconsistencies of the WireController's linkages. */
		virtual void doFixUp()=0;

		/*! \remarks This method will return the parent of the wire controller. */
		virtual ReferenceTarget*	get_parent()=0;
		...
		BEGIN_FUNCTION_MAP
			...
			RO_PROP_FN(isDriver,	 is_driver,		 TYPE_bool);
			RO_PROP_FN(isDriven,		 is_driven,		 TYPE_bool);
			RO_PROP_FN(isTwoWay,	 is_two_way,	 TYPE_bool);
			PROP_FNS(getDrivenAnimation, get_driven_animation, setDrivenAnimation, set_driven_animation, TYPE_CONTROL);
			FN_0(validate,			 TYPE_bool,		 doValidate);
			VFN_0(fixUp,							 doFixUp);
			RO_PROP_FN(getParent,	 get_parent,	 TYPE_REFTARG);
		END_FUNCTION_MAP
};
