...
public:
	...
	*/
	...
	/*! \brief Copy constructor. Copies the contents values between 2 iterators.
		\param[in] begin - iterator that defines where to start copying from
		\param[in] end - iterator that defines when to stop copying
	*/
	...
	*/
	...
	//! \brief Accesses the begin iterator, which is a plain pointer in this case.
	T* begin();
	//! \brief Accesses the const begin iterator, which is a plain pointer in this case.
	const T* begin() const;
	...
	*/
	...
	//! \brief Accesses the end iterator (one past last valid element). Do not dereference.
	T* end();
	//! \brief Accesses the const end iterator (one past last valid element). Do not dereference.
	const T* end() const;
