...
public:
	...
	PFExport virtual MSTR SubAnimName(int i, bool localized = true) override;
...
public:
	PFExport virtual RefResult NotifyRefChanged(const Interval& changeInt,RefTargetHandle hTarget,PartID& partID, RefMessage message, BOOL propagate);
	...
	// From Reference Target; needed by SlateModelWrapper
	PFExport virtual void NotifyTarget( int message, ReferenceMaker* hMaker ) { SlateModelWrapper::RefMakerNotifyTarget( message, hMaker ); }
	...
	// From Object
	virtual void InitNodeName(MSTR& s) { s = GetObjectName(true); }
	...
	// From Animatable
	virtual void GetClassName(MSTR& s, bool localized = true) const override { UNUSED_PARAM(s); UNUSED_PARAM(localized); }
