...
/*! \sa  Class BaseInterface, Class IObject, ~{ %Function Publishing }~.\n\n
\par Description:
...
public:
	...
	/*! \remarks Returns a pointer to the interface who is specified.
	Interfaces that wish to support this helper function must specify a 'static constexpr Interface_ID ID = ...'
	variable. This allows the helper function to determine at compilation time which ID to use for the query.
	*/
	template<class InterfaceType>
	InterfaceType* GetTypedInterface()
	{
		return static_cast<InterfaceType*>(GetInterface(InterfaceType::ID));
	}
};
...
//  
/*! \sa  Class InterfaceServer, Class InterfaceNotifyCallback,  Class FPInterface, ~{ %Function Publishing }~.\n\n
\par Description:
...
public:
	...
	The interface is only good for one call. The release is implied so a call
	to release is not required.\n\n
	<b>wantsRelease</b>\n\n
	The clients are controlling the lifetime, so the interface needs a
	Release() when the client has finished.\n\n
	...
	\par Default Implementation:
	<b>{ return NULL; }</b> */
	virtual BaseInterface* CloneInterface(void* remapDir = nullptr){ UNUSED_PARAM(remapDir); return nullptr; }
};
...
/*! \sa  Class BaseInterface, ~{ %Function Publishing }~.\n\n
\par Description:
...
public:
	...
	virtual BaseInterface* GetInterface(Interface_ID id) { UNUSED_PARAM(id); return nullptr; }
};
