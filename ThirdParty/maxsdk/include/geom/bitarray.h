...
class BitArray
{
	...
	enum
	{
		kMAX_LOCALBITS = CHAR_BIT * sizeof(uint64_t),
	};
...
private:
	enum
	{
		BITS_PER_DWORD_PTR = (CHAR_BIT * sizeof(uint64_t)),
	...
	{
		return ((long long)(k) + BITS_PER_DWORD_PTR - 1) / BITS_PER_DWORD_PTR;
	}
	...
	inline BitArray(uint64_t localBits_, long numBits_, bool zeroHighBits = false)
