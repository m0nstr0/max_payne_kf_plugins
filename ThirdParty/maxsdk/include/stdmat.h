//*********************************************************************/
// Copyright (c) 1996-2020 Autodesk, Inc.
// All rights reserved.
//
...
class StdMat: public Mtl {
	public:
//! \name Setting Material Properties
//! @{
...
//! \name Retrieving Material Properties
//! @{
...
public:
	...
	/*!	Changes the file name of the image to be used.
		\param[in] pName - The new file name.
	...
	*/
	virtual void SetMapName(const MCHAR *pName, bool isUIAction = false)=0;
	...
	*/
	...
	virtual Bitmap *GetBitmap(TimeValue t) { return nullptr; }
	#pragma warning(pop)
...
class MultiMtl: public Mtl {
	...
	<b>MSTR \&subMtlName</b>\n\n
	The name is returned here. */
	virtual void SetSubMtlAndName(int mtlid, Mtl *m, const MSTR& subMtlName) = 0;
	...
	\param pName - The name of the new sub material. */
	virtual void AddMtl(ReferenceTarget *rt, int mtlid, const MCHAR *pName)=0;
...
class Tex3D: public Texmap {
	...
	virtual void ReadSXPData(const MCHAR *pName, void *sxpdata)=0;
	};
...
//! \brief Create a Physical Material (or, optionally, a legacy StdMtl2)
/*! Create new instances of the Physical Material, optionally depending
    on a 3dsmax.ini configuration key. If no config key is passed,
    the creation is unconditional. If a key name is passed, that key
    is searched in 3dsmax.ini's [LegacyMaterial] section, and if
    existing and set to 1, a legacy standard material is returned
    instead. This allows restoring legacy behaviour inside 3ds Max 
    if that is necessary for some particular users reason.

    The optional legacy parameter can point to a bool that will be set 
    to true if the legacy fallback is used, meaning a StdMtl2 is returned.

    \note This returns EITHER a Physical Material OR a Legacy Standard
    material. No other types of materials are ever returned.
*/
CoreExport Mtl* NewPhysicalMaterial(const MCHAR *configkey = nullptr, bool *legacy = nullptr);

/*! \brief Create a material of the type defined in the "market defaults"
    (or optionally, a StdMtl2 as a fallback)
*/
/*! Create new instances the configured "market default" mtl, optionally
    depending on a 3dsmax.ini configuration key. If no config key is passed,
    the creation is unconditional. If a key name is passed, that key
    is searched in 3dsmax.ini's [LegacyMaterial] section, and if
    existing and set to 1, a legacy standard material is returned
    instead. This allows restoring legacy behaviour inside 3ds Max 
    if that is necessary for some particular users reason.
    \note This can return ANY material, whatever is configured in
    the market defaults.
*/
CoreExport Mtl *NewDefaultMaterial(const MCHAR *configkey = nullptr);

//! \brief Return the primary texture slot for a material
/*! Function returns the primary texture slot ID for a material, generally
    the diffuse slot or similar. If the material derives from StdMtl2, this 
    is equivalent to calling mtl->StdIDtoChannel(ID_DI).
*/
