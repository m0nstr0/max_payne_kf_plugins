...
/** \deprecated This class is deprecated: Please use IColorPipelineMgr which 
provides better color management functionality.  */
/** Deprecated interface for basic color management. The differences between
IColorCorrectionMgr and GammaManager is the number of channels corrected and the
correction types (Autodesk LUT, gamma). IColorCorrectionMgr manages 3 channels,
so that all Autodesk LUTs can be loaded. GammaManager can still be used to
change or access the display gamma value or to apply gamma correction on a color
BUT it is now highly recommended to use IColorCorrectionMgr.  Only the use of
IColorCorrectionMgr will provide the use of Autodesk LUTs. Here is a short
description of how this class may be used: 
...
public:
		...
		enum CorrectionMode {
			kLUT_DEPRECATED,	//!< %Color correction using Autodesk LUT
		...
		*/
		virtual void SetColorCorrectionPrefMode(const CorrectionMode mode)=0;
		...
		*/
		virtual CorrectionMode GetColorCorrectionPrefMode() const =0;
		...
		*/
		virtual void SetGamma(float newGamma)=0;
