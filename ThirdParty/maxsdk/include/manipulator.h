...
public:
    ...
    ManipExport Plane(const Point3& normal, const Point3& point);
    /*! \remarks Constructor.\n\n
    ...
    The three points the plane should pass through.\n\n
      */
    ManipExport Plane(const Point3& p1, const Point3& p2, const Point3& p3);
    /*! \remarks Constructor. */
    ...
    ManipExport bool Intersect(const Ray& ray, Point3& intersectionPoint) const;
    ...
    */
    ...
    ManipExport float   GetPlaneConstant() const { return mD; }
...
public:
    ...
    ManipExport void AppendPoint(const Point3& p) {
...
public:
	...
	virtual Mesh* MakeSphere(const Point3& pos, float radius, int segments)=0;
	...
	\return  A pointer to the resulting mesh. */
	virtual Mesh* MakeTorus(const Point3& pos, float radius, float radius2, int segs, int sides)=0;
	...
	\return  A pointer to the resulting mesh. */
	virtual Mesh* MakeBox(const Point3& pos, float l, float w, float h, int lsegs, int wsegs, int hsegs)=0;
	...
	virtual Plane* MakePlane(const Point3& p1, const Point3& p2, const Point3& p3)=0;
	/*! \remarks This method creates a plane gizmo.
	\par Parameters:
	<b>Point3\& normal</b>\n\n
	The normal of the plane\n\n
	...
	virtual Plane* MakePlane(const Point3& normal, const Point3& point)=0;
    ...
    virtual GizmoShape* MakeCircle(const Point3& center, float radius, int segments)=0;

	// Object manipulator display control

	/* \remarks This enum controls whether all manipulators for a given selected object
	are displayed (kAllManipulators) or just those whose object or modifier is active
	(kActiveManipulators) 
    
    For example: The TextPlus object and Sphere object display manipulators that control
    attributes of those base objects. Modifiers applied to these objects may display their
    own manipulators. When a modifier is applied to these objects and the modifier is
    active in the Modify panel, 'kAllManipulators' will not only display the modifier's
    manipulators but will also display those of the base object. This can cause visual
    clutter, confusion and clicks intended for the modifier's manipulators can be picked
    up by the base object, or vice-versa, resulting in user frustration.

    To alleviate this situation, the user can select the 'kActiveManipulators' mode,
    which tells 3ds Max to only display the manipulators of the object or modifier which
    is currently active in the Modify panel. In this setting, working in the modifier
    applied to TextPlus or Sphere, only the modifier's manipulators will be available.
    When working at the base object level, only the base object's manipulators will be
    available.
    */
	enum ObjectManipulatorDisplayMode {
		kAllManipulators = 0,
	...
	/*! \remarks This method will get the current state of the Object Manipulator Mode.
	\return  The current object manipulator display mode. \see ObjectManipulatorDisplayMode*/
	virtual ObjectManipulatorDisplayMode GetObjectManipulatorMode() = 0;

	/*! \remarks This method will set the current state of the Object Manipulator Mode.
	\par Parameters:
	<b>int newMode</b>\n\n
	The new mode to use. \see ObjectManipulatorDisplayMode\n\n
	\return  The previous object manipulator display mode. \see ObjectManipulatorDisplayMode */
	...
	enum { makeSphere, makeTorus, makeBox, makePlane, makePlaneFromPts,
           ...
           makeGizmoShape, makeCircle, getObjectManipDisplayMode,
...
public:
	...
	*/
	...
	/*! \remarks This method gets called when the right mouse button is first
	pressed down within the manipulator context.
	\par Parameters:
	<b>TimeValue t</b>\n\n
	The time to display the object.\n\n
	...
	/*! \remarks This method gets called when the right mouse button is released
	within the manipulator context, and thus signals the end of the
	manipulation.
	\par Parameters:
	<b>TimeValue t</b>\n\n
...
public:
    ...
    // Animatable methods
    ManipExport void GetClassName(MSTR& s, bool localized = true) const override {s = GetObjectName(localized);}
