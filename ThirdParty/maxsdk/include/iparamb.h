...
// When a client of a param block receives the \ref REFMSG_GET_PARAM_NAME_LOCALIZED message or
// the \ref REFMSG_GET_PARAM_NAME_NONLOCALIZED message, the partID field is set to point at one of these structures.
...
receives the \ref REFMSG_GET_PARAM_NAME_LOCALIZED message or
the \ref REFMSG_GET_PARAM_NAME_NONLOCALIZED message, the partID field is set to
...
for more detail on how this is done. 
 <b>ParamID pb2_id</b>\n\n
This is an ID used to identify this parameter in a ParamBlockDesc2.
There are 2 special values for this member:
-1 - This ParamBlockDesc parameter is not used in the ParamBlockDesc2
-2 - Use the 'id' member value as the ParamBlockDesc2 id.
Otherwise, the value specifies the ParamBlockDesc2 parameter id that corresponds to this parameter.
This member is used by function CopyParamBlock2ToParamBlock, ProcessPB2ToPB1SaveToPrevious, and ParamBlock2PLCB 
when copying parameters between IParamBlock and IParamBlock2 instances
*/
...
public:
	...
	DWORD id;
	ParamID pb2_id;
	ParamBlockDescID() : type(TYPE_FLOAT), user(nullptr), animatable(FALSE), id(0), pb2_id(-2) {}
	ParamBlockDescID(ParamType type, UserType* user, BOOL animatable, DWORD id) : type(type), user(user), animatable(animatable), id(id), pb2_id(-2) {}
	ParamBlockDescID(ParamType type, UserType* user, BOOL animatable, DWORD id, ParamID pb2_id) : type(type), user(user), animatable(animatable), id(id), pb2_id(pb2_id) {}
};
...
class IParamArray : public MaxHeapOperators {
	public:
		...
		\param i - This is the index into the virtual array of the value to retrieve. 
		\param t - This is the time at which to retrieve the value. For constants pass
		0. 
		\param v - This is the value to retrieve.
		\param ivalid - The interval into which the validity of the evaluated parameters is intersected.
		...
		// Use this overload when validity interval is not relevant
		template <typename T>
		BOOL GetValue(int i, TimeValue t, T& v, Interval&& valid = FOREVER) { return GetValue(i, t, v, valid); }
		// If it is a param block, this will get a pointer to it, otherwise it will return NULL.
		// Note that casting won't work because of multiple inheritance.
...
class IParamBlock : 			
	...
	public:
		...
		*/
		...
		BOOL GetValue(int i, TimeValue t, ValueType& v, Interval&& valid = FOREVER) { return GetValue(i, t, v, valid); }
