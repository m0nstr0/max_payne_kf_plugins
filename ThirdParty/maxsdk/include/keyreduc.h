...
{
	KEYREDUCE_ABORT = -1,	// Stops processing and undoes any key reduction
	KEYREDUCE_STOP = 0,		// Stops processing, but keeps any reduction done so far
	KEYREDUCE_CONTINUE = 1,	// Keeps going.
};

// A callback so progress can be made during key reduction
/*! \sa  ::ApplyKeyReduction()
\par Description:
...
class KeyReduceStatus: public MaxHeapOperators {
	public:
		...
		Keeps going. */
		virtual KeyReduceResult Progress(int p) = 0;
	};
 ...
 */
CoreExport KeyReduceResult ApplyKeyReduction(
