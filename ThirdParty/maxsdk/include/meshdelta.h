...
or more of the vertices a face uses. It can also alter the visibility of the
...
public:
	...
	DWORD f;
	/*! Specifies which vertices to remap. One or more of the following values:
	...
	DWORD flags;
	/*! Array of vertex indices. These indicate which vertex is used by each specified
	...
	\param vertexIndexArray - The array of vertex indices. */
	...
	\param ff -  The face whose vertices are remapped. */
	...
	\param tf - The texture face whose tVerts are remapped by the verts of this FaceRemap 
	...
	FaceRemap passed.
	\param fr - The FaceRemap whose flags and verts are assigned.  */
...
visibility of the face's edges, its hidden state and/or its material ID. 
\sa  Class Mesh, Class Face,  Class Point3. */
...
public:
	...
	this FaceChange object to control what's altered.
	\par Parameters:
	<b>Face \&ff</b>
	The face to change. */
	DllExport void Apply (Face &ff);
...
updates the smoothing group information in the face.
\sa  Class Mesh, Class Face,  Class Point3. */
...
public:
	...
	are stored in the order created. This must match driver MeshDelta fCreate in size.*/
	Tab<TVFace> fCreate;
...
//! \name Initialization and Clean-up
//! @{   
...
/** \name Composition and Operators
 * MeshDeltas can be multiplied together like Transforms before being applied to a mesh.
 * This is especially useful in Edit Mesh, where a single MeshDelta is used to store the current
 * "state" of the Edit Mesh and extra MeshDeltas are created for each operation and added
 * on to the main MeshDelta.
 * 
 * An example of this is as follows: given a Mesh m that we want to divide a face on, and then
 * we want to break the vertex created in the face divide. These operations are adequately handled by
 * the DivideFace and BreakVerts methods independently, but to combine the two into one
 * operation, we must compose two MeshDeltas.
 * 
 * \code
 ...
 * \endcode
 *  
 * If all we wanted was to perform these operations on the mesh given, we could have done so without making the composition.
 * The point here is to be able to construct complex MeshDeltas representing a series of user operations while remaining
 * flexible enough to respond to changes in the actual mesh given as input.
 */
///@{   
 ...
 * All of them may be safely performed on MeshDeltas that are already quite complex.
 * Those that accept DWORD indices require output mesh indices as all operations are appended to the end of the existing delta.
 */
///@{
   ...
   */
   ...
   appropriate flags and vertices should be passed in remapFlags and v arguments.
   ...
   <b>BOOL bCopySel=TRUE</b>
   If want the new edges and face to have their selection bits set from the original
   face, set this value to TRUE. Otherwise the new face will have their selection state cleared. 
...
/** \name Advanced Additive Operations
 * The following are more complex operations. The mesh given is expected to be the result of the current MeshDelta.
 * They must all work no matter how complex the current MeshDelta is, but they have not been extensively tested on complex MeshDeltas.
 */
///@{
...
///@{   
   ...
   */
   ...
   DllExport DWORD Cut (Mesh & m, DWORD ed1, float prop1, DWORD ed2, float prop2,
      const Point3 & norm, bool fixNeighbors=TRUE, bool split=FALSE);
   ...
   A pointer to the adjacent edge list for this mesh. If NULL, the method
   constructs its own AdjEdgeList.
   \return  Returns TRUE if any faces were sliced, FALSE if none were sliced.*/
   DllExport BOOL Slice (Mesh & m, Point3 N, float off, bool sep=FALSE, bool remove=FALSE, BitArray *fslice=NULL, AdjEdgeList *ae=NULL);
///@}

private:

   // Clears any face index maps to be used to propagate specified normals from
   // source to target faces in a relevant operation. Called by any oepration
   // which may invalidate such a map.
   void ClearFaceNormalIndexMaps();
};
