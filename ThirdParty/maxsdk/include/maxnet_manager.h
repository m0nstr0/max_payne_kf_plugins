...
//! @{
		...
		A pointer to a string to receive the message. This string should be <b>MAX_PATH</b> long.\n\n
		<b>size_t status_text_size</b>\n\n
		The size in characters of the status_text buffer. If the buffer is not big enough to hold the output (including a null terminator), the result is truncated.\n\n */
		virtual void	GetJobServerStatus		( HJOB hJob, HSERVER hServer, MCHAR* status_text, size_t status_text_size )=0;
...
class MaxNetworkInterface: public MaxHeapOperators {
	public:
		...
		virtual	bool	GetCurrentRenderer		( MCHAR* name, size_t nameSize, DWORD* id1, DWORD* id2)=0;
};
...
class MaxNetworkInterface2 : public MaxNetworkInterface {
	...
	public:
		...
		void	SetMAXVersion(int ver)				{this->version = ver;}
