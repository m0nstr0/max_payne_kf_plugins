...
public: 
	...
	*/
	...
	\return true if we are using the nearest edge/vertex with preview selection.
	*/
	...
	\param useNearestPreviewSelection - Whether we are using the nearest edge/vertex with preview selection.
	*/
	 ...
	 * Modifiers and objects can query this property to determine if the user wishes
	 * to ignore back facing verts and edges in shaded viewport picking. Faces can still
	 * be selected if you wait a few seconds and click without moving the mouse.
	 * Area/region selection can select back facing verts, edges, and faces.
	\return true if we are ignoring back facing verts and edges in shaded view mode.
	*/
	...
	\param ignoreBackVertsAndEdgesInShadedViewportPicking - Whether we will ignore back facing verts and edges in shaded view mode.
	*/
