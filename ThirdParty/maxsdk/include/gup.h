...
class GUPHandler : public InterfaceServer {
	...
	public:
		...
		void		SetID			( Class_ID cid )	{ id = cid;	}
		Class_ID	GetID			( )					{ return id;}
		void		SetInstance		( GUP *ins )		{ instance = ins;  }
		GUP*		GetInstance		( )					{ return instance; }

		GUP*		Gup				( )					{ return instance; }
...
class GUPList: public MaxSDK::Array<GUPHandler>
{
	public:
		/*! \remarks Constructor. */
		GUPExport			GUPList				( )	{ ; }
...
class GUP : public InterfaceServer {
	...
	public:
		...
		*/
		...
		GUPExport virtual	bool			ExecuteStringScript	( const MCHAR *string, MAXScript::ScriptSource scriptSource);

		//! \brief Executes the given script file
		/*! \param[in] file - The fully qualified path to the existing file. This can be maxscript files (*.ms, .mxs), maxscript zip files (*.mzp), 
		...
		\note - if captured_error_message is not null, no error dialog is displayed if an error occurs.
		\note - if the script file name is persisted in the scene file, the script file should be handled as an Asset and executed using 
		ExecuteScriptAsset(const MaxSDK::AssetManagement::AssetUser& file, MSTR* capturedErrorMessage = nullptr)
		\remarks Implemented by the System.*/
		GUPExport virtual	bool			ExecuteFileScript	( const MCHAR *file, MSTR* captured_error_message = nullptr );
		...
		/*! \param[in] asset - The asset that points to the existing file. This can be maxscript files (*.ms, .mxs), maxscript zip files (*.mzp), 
		encrypted maxscript files (*.mse), or python files (*.py, *.pyc, *.pyw).
		\param[in, out] captured_error_message - String for capturing error message, can be nullptr. 
		\return - true if the script was successfully executed, false if not. Note that this does not reflect the result of the script.
		\note - if captured_error_message is not null, no error dialog is displayed if an error occurs.
		\remarks Implemented by the System.*/
		GUPExport virtual	bool			ExecuteScriptAsset	( const MaxSDK::AssetManagement::AssetUser& asset, MSTR* captured_error_message = nullptr );
      ...
      //            it has to be a DWORD_PTR.
		...
		GUPExport virtual	void			DeleteThis	( ) = 0;
};
