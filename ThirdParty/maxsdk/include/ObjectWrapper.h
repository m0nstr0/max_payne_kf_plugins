...
class ObjectWrapper: public MaxHeapOperators {
	...
	//-- Sub-object selection support
		...
		CoreExport void		SetVertSel(const BitArray& sel);
		...
		//! \param sel - The selection bits
		CoreExport void		SetFaceSel(const BitArray& sel );
		...
		\param sel - The selection bits */
		CoreExport void		SetElemSel(const BitArray& sel );
		...
		//! \param selLevel - The selection level. See the selection enum values above
		//! \param sel - The selection bits
		CoreExport void		SetSel( int selLevel, const BitArray& sel ); //sets the selection of the given type
		...
		CoreExport void		FaceToElemSel( BitArray& elemSel, const BitArray& faceSel ); //convert face to elem selection
	...
	//-- Hit testing and ray intersection
		...
		void render(GraphicsWindow* gw, Material* ma, RECT* rp, int compFlags, int numMat = 1) { Render(gw, ma, rp, compFlags, numMat); }
