...
enum JOB_TEXT_TYPE
{
	...
	JOB_TEXT_CMB_OUTPUT,			//!< \deprecated This has been deprecated as of 3ds Max 2023, it's not used any more.
	#pragma deprecated("JOB_TEXT_CMB_OUTPUT")
	JOB_TEXT_RENDER_ELEMENT = 70,	//!< Render Elements
...
\brief The CJobText class stores job information which is of a dynamic nature or of variable length.
*/
class MAXNETEXPORT CJobText: public MaxHeapOperators
{
	...
	public:
		...
		/*!
		\remarks Destructor. The CJobText buffers will be deallocated.
		*/
		~CJobText();

		/*!
		\remarks This method return the number of JobText buffers in the list. Note: Developers should use Job.jobtextcount to find out how many elements there are.
		*/
		int      Count();

		/*!
		\remarks      This method will add another JobText buffer.
		\param[in] jt A pointer to the JobText buffer to add.
		...
		*/
		...
		\param[in] idx   The position of the first index to be deleted.
		\param[in] count The number of entries to delete.
		*/
		void     Delete(int idx, int count = 1);

		/*!
		\remarks This method will reset and deallocate the CJobText buffers.
		*/
		void     Reset();

		/*!
		\remarks This method will return a pointer to the actual JobText buffer.
		*/
		JobText* Buffer();
		...
		*/
		...
		/*!
		\remarks     This access operator returns a reference to a JobText entry.
		\param[in] i The index of the JobText buffer to return.
		*/
		JobText& operator[](const int i);

		/*!
		\remarks         This method allows you to search for a JobText entry by its type. Refer to the list of Job Text types for details.
		\param[in] tp    The Job Text type you wish to find.
		\param[in] start The start position from which to initiate the search process.
		\return          The index of the entry which was found, or -1 if not found.
		*/
		int      FindJobText(JOB_TEXT_TYPE tp, int start = 0);

		/*!
		\remarks              This method retrieves an index based on it's Text Type. Refer to JOB_TEXT_TYPE for details.
		\param[out] text      A pointer to the text buffer which will be filled in by the method.
		\param[in]  textSize  The size in characters of the text buffer which will be filled in by the method. If the text buffer is not big enough to hold the output (including a null terminator), the result is truncated.
		\param[in]  type      The Job Text type you wish to find.
		\param[in]  start     The start position from which to retrieve the text item.
		\param[in]  idx       A pointer to the index found.
		...
		*/
		...
		/*!
		\remarks             This method will retrieve the user name associated with a job.
		\param[out] user     The user name which was retrieved.
		\param[in]  userSize The size in characters of the user buffer which will be filled in by the method. If the user buffer is not big enough to hold the output (including a null terminator), the result is truncated.
		\return              TRUE if the user name was retrieved, otherwise FALSE.
		*/
		bool     GetUser(MCHAR* user, size_t userSize);

		/*!
		\remarks                 This method will retrieve the computer name associated with a job.
		\param[out] computer     The computer name which was retrieved.
		\param[in]  computerSize The size in characters of the computer buffer which will be filled in by the method. If the computer buffer is not big enough to hold the output (including a null terminator), the result is truncated.
		\return                  TRUE if the computer name was retrieved, otherwise FALSE.
		*/
		bool     GetComputer(MCHAR* computer, size_t computerSize);

		/*!
		\remarks              This method will retrieve the frame sequence (such as the "1,2,4,5-40" types).
		\param[out] frames    The frames retrieved.
		\param[in] framesSize The size in characters of the frames buffer which will be filled in by the method. If the frames buffer is not big enough to hold the output (including a null terminator), the result is truncated.
		\return               TRUE if the frame sequence string was retrieved, otherwise FALSE. */
		bool     GetFrames(MCHAR* frames, size_t framesSize);

		/*!
		\remarks             This method will retrieve the Manager's network share associated with a job.
		\param[out] share    The network share which was retrieved.
		\param[in] shareSize The size in characters of the share buffer which will be filled in by the method. If the share buffer is not big enough to hold the output (including a null terminator), the result is truncated.
...
/*! \sa Class MaxNetManager, Structure AlertData, Structure MaxJob
\remarks This structure is available in release 4.0 and later only. \n\n
