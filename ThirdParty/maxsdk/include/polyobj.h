...
// must run your plugin with 3ds max version 4.2 or higher.
	...
	int NumMapsUsed () { return mm.MNum(); }
	...
	void GetClassName(MSTR& s, bool localized = true) const override { UNUSED_PARAM(localized); s = MSTR(_M("PolyMeshObject")); }
	...
	const MCHAR *GetObjectName(bool localized = true) const override { return _M("PolyMesh"); }
	PolyLibExport void RescaleWorldUnits(float f);

	// IO
	PolyLibExport IOResult Save(ISave *isave) { return Save(isave, ALL_CHANNELS); }
	PolyLibExport IOResult Load(ILoad *iload) { return Load(iload, ALL_CHANNELS); }
	PolyLibExport IOResult Save(ISave *isave, ChannelMask channels);
	PolyLibExport IOResult Load(ILoad *iload, ChannelMask channels);
	...
	void SetDisplacementParameters (const TessApprox & params) { mDispApprox = params; }
...
PolyLibExport bool PreparePolyObjectDisplay(
		MNMesh& mesh, const MaxSDK::Graphics::UpdateDisplayContext& updateDisplayContext);
