 ...
 *
 * The Path interface provides operations for path composition (Path::Append and 
 * Path::Prepend ) and decomposition (Path::StripToTopParent, Path::StripToLeaf, Path::RemoveTopParent, 
 * Path::RemoveLeaf), query methods for determining information about a Path,
 ...
 * may be surprising to a user.  If the user wishes to construct a path with 
...
public:
    ...
    * e.g. "\\abc\\def" is rooted at backslash.
	...
	/*! \brief Queries whether the file or directory is rooted to the specified folder. 
	This works on both files and directories.
	\return - true if the file or directory is rooted to the specified folder and false otherwise */
