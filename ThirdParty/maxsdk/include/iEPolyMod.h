...
// Edge chamfer types (legacy)
enum {
	EP_STANDARD_EDGE_CHAMFER = 0,	// (Legacy) Mitering type = tri
	EP_QUAD_EDGE_CHAMFER			// (Legacy) Mitering type = quad
};
...
enum
{
	EP_EDGE_CHAMFER_QUAD_MITER = 0,    // Legacy "Quad" Chamfer
	EP_EDGE_CHAMFER_TRI_MITER,         // Legacy "Standard" Chamfer
	EP_EDGE_CHAMFER_UNIFORM_MITER,     // Maya Uniform style (Legacy Quad Chamfer + Quad Intersections)
    EP_EDGE_CHAMFER_RADIAL_MITER,      // Radial style
    EP_EDGE_CHAMFER_PATCH_MITER        // Patch style
};
...
enum epolyModParameters {
	...
	epm_edge_chamfer_mitering_type,		// int - mitering type -- quad, tri, uniform, patch, or radial
	epm_edge_chamfer_end_bias,			// float - the end bias amount for the miter
	epm_edge_chamfer_radial_bias,		// float - biasing factor controlling the radius of chamfered corners
	epm_edge_chamfer_depth,				// float - the depth of the chamfer (negative values are concave, 0 is linear, positive values are convex)

	// New for 2021
	epm_vertex_chamfer_num_segments,        // int - number of segments, controlling refinement of the chamfered surface mesh
	epm_vertex_chamfer_depth,               // float - the depth of the chamfer (negative values are concave, 0 is linear, positive values are convex)
	epm_vertex_chamfer_smooth,              // bool - when true, adjusts the smoothing groups of the post-chamfer geometry 
	epm_vertex_chamfer_smooth_type,         // int - controls the smoothing approach (see EP_QUAD_SMOOTH_XXX)
	epm_vertex_chamfer_smooth_threshold,    // float - angle threshold, in degrees, to use when adjusting smoothing groups

    // New for 2022
    epm_enable_enhanced_hotkey_extrude,     // bool - when true, enables postprocessing to clean up geometry and topology during hotkey extrude operations

	// New for 2022.1
	epm_ignore_occluded,					// bool - ignores occluded sub-objects in selections

	// New for 2024
	epm_enable_enhanced_retriangulation     // bool - when true, enables the enhanced triangulation algorithm implemented in MaxSDK::BuilDiagonals
};
...
enum epolyModButtonOp { ep_op_null,
    ...
    // New for 3ds Max 2022
    ep_op_extrude_face_postprocessed, ep_op_finalize_extrude_face_postprocessed
};
...
enum epolyModMethods {
	...
	// New for 2021
	epmod_get_edge_chamfer_type, epmod_set_edge_chamfer_type, // Legacy edge chamfer properties support
	epmod_get_edge_chamfer_quad_intersections, epmod_set_edge_chamfer_quad_intersections // Legacy edge chamfer properties support
};
...
public:
	...
	/// 
	///  \param meshSelectionLevel  The MNMesh selection level, such as MNM_SL_VERTEX for vertex or MNM_SL_CURRENT for "the current level". 
	...
	virtual BitArray *EpModGetSelection (int meshSelectionLevel, INode *pNode=NULL) { return NULL; }
	...
	virtual bool EpModSetSelection (int meshSelectionLevel, BitArray & selection, INode *pNode=NULL) { return false; }
	...
	///  \param pNode  If this Edit Poly modifier is instanced across multiple nodes, this parameter specifies which
	/// node we want to set selection in. 
	virtual bool EpModSelect (int meshSelectionLevel, BitArray & selection, bool invert=false, bool select=true, INode *pNode=NULL) { return false; }
	...
	/// 
	virtual Matrix3 EpGetSlicePlaneTM(TimeValue t) { return Matrix3{}; }
	...
	virtual Matrix3 EpModGetNodeTM(TimeValue t, INode *node = NULL) { return Matrix3{}; }
	...
	///  \brief 
	/// Lists the entire history of applied operations to the Scripting Listener window.  This is not quite
