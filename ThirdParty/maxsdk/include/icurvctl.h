...
#define IS_BEZIERCORNER(flags)  ( ( (flags) & CURVEP_BEZIER) && ((flags) & CURVEP_CORNER) )
...
public:
	...
	\param fX - The input X value.
	\param ivalid - The interval into which the validity of the evaluated parameters is intersected.
	\param UseLookupTable - If TRUE a lookup table is used to get the value (for speed). If FALSE the
	value is computed.
	...
	float GetValue(TimeValue t, float fX, Interval&&, BOOL UseLookupTable = FALSE) { return GetValue(t, fX, UseLookupTable); }
	...
	*/
	...
	virtual int Insert(int where, const CurvePoint& p) = 0;
//watje
...
//would often get in the way
	...
	\return  Nonzero if the point was inserted; otherwise zero. */
	virtual int Insert(int where, const CurvePoint& p, BOOL do_not_hold) = 0;
