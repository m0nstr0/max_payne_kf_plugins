...
public:
	...
	*/
	...
	//! \brief Get the list of unique filenames that are used by the given Animatable.
	/*! \param[in] animatable The animatable for which the file dependencies are to be listed.
		...
		hierarchy of 'animatable'; set to false to limit the list to 'animatable'.
		\param[in] caseSensitiveFilenames Set to false for method to handle asset filenames in a case 
		insensitive manner when comparing filenames for determining uniqueness.
		\param[in] getResolvedFileNames Set to false for method to collect the assets' specified filenames.
		If true, if the filename can be resolved then the resolved filename is collected otherwise the
		specified filename is collected.
	*/
	virtual int				GetDependencyFileList(Animatable* animatable, MaxSDK::Array<MSTR>& fileList, bool includeRefHierarchy, 
								bool caseSensitiveFilenames = true, bool getResolvedFileNames = true) = 0;
	...
	*/
	...
	\param[in] useResolvedFileNames Set to false if fileList contains unresolved filenames and only files in 
	the dialog that match that unresolved filename will be selected.
	If true, all files in the dialog that resolve to the filenames in the fileList will be selected. If a 
	file in the dialog cannot be resolved the unresolved filename will be used for comparison.
	*/
