...
{;
    ...
    //! \brief Extends INotificationManager 
    class INotificationManager2 : public INotificationManager
    {
    public:
        /** Notifies the system of a viewport event.
            This method is used internally by the the application to broadcast a change in the
            viewport. The notification system relies on this method to catch such
            events because there are no other, pre-existing methods (such as through BroadcastNotification())
            for broadcasting such events.
            @param eventType: The type of event to be broadcast. 
            @param viewID: The id of viewport with the change*/
        virtual void NotifyViewEvent(const ViewEventType eventType, const int viewID) const = 0;
    };
