	...
	namespace  GeomBind 
	{
		...
		*/
		...
		public:
			...
			SEEXPORT static ITransformHierarchy * create( );
			/*!  This disposes an instance of an ITransformHierarchy		   		*/
			SEEXPORT static void                  dispose( ITransformHierarchy * );
		...
		*/
		...
		public:
			...
			*/	
			SEEXPORT virtual void appendTriangle(
				unsigned i0,
				unsigned i1,
				unsigned i2
				) = 0;
		...
		*/
		...
		public:
			...
			*/
			SEEXPORT virtual unsigned count( ) const = 0;
			...
			*/
			SEEXPORT virtual Influence influence(
				unsigned vertex,
				unsigned influence
				) const = 0;
		...
		*/
		class IProgress
		{
			public:
				...
				*/
				SEEXPORT virtual IProgress & start(const char * description = 0, int max = 100, int val = 0 ) = 0;
				/*!  Called at the start of a major operation */
				SEEXPORT virtual IProgress & end() = 0;
				/*!  Sets the max and current time left
				\param max is the max value for the amount of time taken
				\param value is the current time
				*/
				SEEXPORT virtual IProgress & setMaxValue( int max, int value = 0 ) = 0;
				/*!  Sets the current time left
				\param value is the current time left
				*/
				SEEXPORT virtual bool        setValue( const int value ) = 0;
				/*!  returns if the operation should abort			*/
				SEEXPORT virtual bool        cancelOperation() = 0;
		};
		...
		*/
		...
		public:
			...
			SEEXPORT static IContext * create ( );
			/*!  This disposes an instance of a context		   		*/
			SEEXPORT static void       dispose( IContext * );
			/*!  This makes the current OGL context active		   		*/
			SEEXPORT virtual void makeCurrent( ) = 0;
			...
			*/
			SEEXPORT virtual const ISparseVertexWeights * bindSparseGV(
				IMesh                * mesh,
			...
			/*!  This creates skin weights based on a heat map algorithm, restrictions are that the mesh be manifold, only the first mesh
				...
				\param maxInfluences the maximum number of weights per vertex that can be applied
			*/
			SEEXPORT virtual const ISparseVertexWeights * bindHeatmap(
				IMesh               * mesh,
				ITransformHierarchy * hierarchy,
				IProgress           * progress,
				float                 falloff       = 0.0f,
