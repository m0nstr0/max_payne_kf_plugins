...
class ViewExp13 : public ViewExp11
{
	public:
		...
		* \param[inout] pDefaultLights - The array of DefaultLight, it should be initialized before calling this method and it will be filled. 
		* It should have a maximum size of 2 as we use a maximum of 2 lights for a viewport.
		* \param[in] outnumDefaultLights - The size of the pDefaultLights array initialized before that call.
		* \param[in] bApplyGlobalLevel - Tells if we should we apply or not the global tint and level from the Render Environement on the lights data ?
...
public:
	...
	//! \brief the actual proc that does the filtering.
	/*! \deprecated Prior to 2024, the proc that does the filtering was called IsVisible. But the 
	callers to the method and the implementations of the method handled the method as if it was 
	called IsHidden. That is, the method would return false if the node was to be visible, true if hidden.
	In order to reduce confusion, the IsVisible method was sealed and made private, and a new virtual method
	IsHidden was added.
	*/
private:
#pragma warning(push)
#pragma warning(disable : 4481)
	MAX_DEPRECATED virtual BOOL IsVisible(SClass_ID /*sid*/, Class_ID /*cid*/, INode* /*node*/) MAX_SEALED 
	{
		DbgAssert(false); 
		return false;
	}
...
//! @{		
		//! \deprecated This has been deprecated as of 3ds Max 2023.
		//! Please use GetAutoBackupInterface()->GetInterval() in iAutobackup.h instead
		MAX_DEPRECATED virtual float GetAutoBackupTime() final { return 0; }
		//! \deprecated This has been deprecated as of 3ds Max 2023.
		//! Please use GetAutoBackupInterface()->SetInterval(float) in iAutobackup.h instead
		MAX_DEPRECATED virtual void SetAutoBackupTime(float minutes) final {}
		//! \deprecated This has been deprecated as of 3ds Max 2023.
		//! Please use GetAutoBackupInterface()->GetEnable() in iAutobackup.h instead
		MAX_DEPRECATED virtual BOOL AutoBackupEnabled() final { return false; }
		//! \deprecated This has been deprecated as of 3ds Max 2023.
		//! Please use GetAutoBackupInterface()->SetEnable(bool) in iAutobackup.h instead
		MAX_DEPRECATED virtual void EnableAutoBackup(BOOL onOff) final {}
//! @}
...
//! @{
	// watje Gets/Sets the selection filter
		...
		*/
		...
		//! \brief This method checks the display filter at index, and sees if the node, class id, and super class id fail the filter check or not.
		/*! \deprecated Prior to 2024, the method was called DisplayFilterIsNodeVisible. But the 
		callers to the method and the implementations of the method handled the method as if it was 
		called DisplayFilterIsNodeHidden. That is, the method would return false if the node was to be visible, true if hidden.
		In order to reduce confusion, the DisplayFilterIsNodeVisible method was sealed and made private, and a new virtual method
		DisplayFilterIsNodeHidden was added.
		*/
	private:
#pragma warning(push)
#pragma warning(disable : 4481)
		MAX_DEPRECATED virtual BOOL DisplayFilterIsNodeVisible(int /*index*/, int /*sid*/, Class_ID /*cid*/, INode* /*node*/) MAX_SEALED
		{
			DbgAssert(false);
			return false;
		}
...
//! @{		
		...
		*/
		...
		#pragma warning(push)
		#pragma warning(disable : 4481)
		/*! \remarks This is the old method with a typo, consider now using GetRendPixelAspect() instead.
		*/
		MAX_DEPRECATED virtual float GetRendApect() MAX_SEALED { return GetRendPixelAspect();} 
		#pragma warning(pop)
		...
		#pragma warning(push)
		#pragma warning(disable : 4481)
		/*! \remarks This is the old method, consider now using SetRendPixelAspect(float a) instead.
		*/
		MAX_DEPRECATED virtual void SetRendAspect(float a) MAX_SEALED { SetRendPixelAspect(a);} 
		#pragma warning(pop)
