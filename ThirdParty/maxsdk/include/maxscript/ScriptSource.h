...
namespace MAXScript {
	enum class ScriptSource : int
	{
		...
		Dynamic,		//!< the script is dynamically generated at runtime and could contain input from various sources that are interpreted as scripted commands. Therefore dynamic scripts will always execute with restricted security rights, regardless of Safe Scene Script Execution settings as long as Safe Scene Script Execution is enabled
		DynamicAlways	//!< same as Dynamic, except also applies when Safe Scene Script Execution is disabled
	};
}