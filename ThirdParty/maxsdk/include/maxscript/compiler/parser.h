...
using create_fn = RolloutControl* (*)(Value* name, Value* caption, Value** keyparms, int keyparm_count);
...
class Parser : public Value
{
	struct ParserSourceData
	{
		...
		MSTR		linePastTokenEnd; // source content captured during parsing that needs to be written to source_capture after processing resource
...
public:
	...
	static const MCHAR *token_names[];
	static void setup();
	...
	MSTR		tokenValueAsString; // The case sensitive string specifying token_value. Used to support case-sensitive property and method names. 
...
private:
	...
	MAXScript::ScriptSource scriptSource; // the source of the input stream
