...
public:
	...
	const MCHAR*		struct_name;	// packaged in a struct if non-null
				   ...
				   Function() { name = nullptr; struct_name = nullptr; }
	ScripterExport Function(const MCHAR* name, const MCHAR* struct_name=nullptr);
...
#	define		is_function(v) ((v)->_is_function())
	BOOL		_is_function() override { DbgAssert(!is_sourcepositionwrapper(this)); return 1; }
...
/*! Class that uses the RAII idiom to push/pop a CallContext. 
	...
	an exception. 
*/
class [[nodiscard]] ScopedPushCallContext : public MaxSDK::Util::Noncopyable
{
...
public:
	...
	Generic() { }
	ScripterExport Generic(const MCHAR* name, value_vf fn, const MCHAR* struct_name = nullptr);
...
public:
 ...
 ScripterExport MAXScriptFunction(const MCHAR* name, int parm_count, int keyparm_count, Value** keyparms,
								  int local_count, Value* body, HashTable* local_scope, MAXScript::ScriptSource scriptSource, short flags = 0);
 ScripterExport MAXScriptFunction(const MCHAR* name, int parm_count, int keyparm_count, Value** keyparms,
								  int local_count, Value* body, HashTable* local_scope, Array* param_names, 
								  Value* owner, MAXScript::ScriptSource scriptSource, short flags = 0);
				~MAXScriptFunction();
#	define		is_maxscriptfunction(v) ((DbgVerify(!is_sourcepositionwrapper(v)), (v))->tag == class_tag(MAXScriptFunction))
	...
	BOOL		_is_function() override { DbgAssert(!is_sourcepositionwrapper(this)); return TRUE; }
	void		collect() override;
	...
	Value*		apply_no_alloc_frame(Value** arglist, int count, CallContext* cc=nullptr) override;
...
public:
	...
	void	gc_trace() override;
	void	collect() override;
	void	sprin1(CharStream* s) override { fn->sprin1(s); }
	BOOL	_is_function() override { return fn->_is_function(); }

#	define is_structMethod(v) ((DbgVerify(!is_sourcepositionwrapper(v)), (v))->tag == INTERNAL_STRUCT_METHOD_TAG)  // LAM - defect 307069
	Value* classOf_vf(Value** arg_list, int count) override { return fn->classOf_vf(arg_list, count); }
	Value* superClassOf_vf(Value** arg_list, int count) override { return fn->superClassOf_vf(arg_list, count); }
	Value* isKindOf_vf(Value** arg_list, int count) override { return fn->isKindOf_vf(arg_list, count); }
	BOOL   is_kind_of(ValueMetaClass* c) override { return fn->is_kind_of(c); }
	Value* get_property(Value** arg_list, int count) override { return fn->get_property(arg_list, count); }
	Value* eval() override { return Value::eval(); }  // 2019.Update1 change, ensure no sdk break
	Value* apply(Value** arglist, int count, CallContext* cc=nullptr) override;
	Value* apply_no_alloc_frame(Value** arglist, int count, CallContext* cc=nullptr) override; // LAM - 11/16/02
};
...
#	define is_pluginMethod(v) ((DbgVerify(!is_sourcepositionwrapper(v)), (v))->tag == INTERNAL_MSPLUGIN_METHOD_TAG)
	...
	Value* superClassOf_vf(Value** arg_list, int count) override { return fn->superClassOf_vf(arg_list, count); }
	Value* isKindOf_vf(Value** arg_list, int count) override { return fn->isKindOf_vf(arg_list, count); }
	BOOL   is_kind_of(ValueMetaClass* c) override { return (is_pluginMethod(c)) ? 1 : Value::is_kind_of(c); }
	Value* get_property(Value** arg_list, int count) override { return fn->get_property(arg_list, count); }
	Value* eval() override { return Value::eval(); }  // 2019.Update1 change, ensure no sdk break
	Value* apply(Value** arg_list, int count, CallContext* cc=nullptr) override;
	Value* apply_no_alloc_frame(Value** arg_list, int count, CallContext* cc=nullptr) override;
...
public:
	...
	ScripterExport static void	  val_to_FPValue(Value* v, ParamType2 type, FPValue& fpv, FPEnum* e=nullptr);
	ScripterExport static Value*  FPValue_to_val(FPValue& fpv, FPEnum* e=nullptr);
	ScripterExport static void	  release_param(FPValue& fpv, ParamType2 type, Value* v, FPEnum* e=nullptr);
...
public:
	...
	virtual void	val_to_FPValue(Value* v, ParamType2 type, FPValue& fpv, FPEnum* e=nullptr) = 0;
	virtual Value*	FPValue_to_val(FPValue& fpv, FPEnum* e=nullptr, FPValueSubtype subtype=SUBTYPE_NONE) = 0;
	virtual void	release_param(FPValue& fpv, ParamType2 type, Value* v, FPEnum* e=nullptr) = 0;
...
#	define	is_iobject(v) ((DbgVerify(!is_sourcepositionwrapper(v)), (v))->tag == class_tag(IObjectValue))
	...
	MSTR	to_filename() override; // returns guidstring if wraps AssetUser
	void	to_fpvalue(FPValue& v) override { v.iobj = iobj; v.type = TYPE_IOBJECT; }
...
// End of 3ds max 4.2 Extension
	...
	BaseInterface* GetInterface(Interface_ID id) override { return iobj->GetInterface(id); }
...
// End of 3ds max 4.2 Extension
	...
	void	InterfaceDeleted(BaseInterface* bi) override { UNUSED_PARAM(bi); fpi = nullptr; }  

	// accesses methods & props in the interface
	Value*	_get_property(Value* prop);
	Value*	_set_property(Value* prop, Value* val) override;
...
// must run your plugin with 3ds max version 4.2 or higher.
	...
	void	to_fpvalue(FPValue& v) override { v.fpi = fpi; v.type = TYPE_INTERFACE; }

// End of 3ds max 4.2 Extension

	// from InterfaceNotifyCallback
	void	InterfaceDeleted(BaseInterface* bi) override; 
