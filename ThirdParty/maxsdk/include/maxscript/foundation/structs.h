...
public:
	...
	HashTable*	member_fns;					/* functions from def wrapped in StructMethod, filled as functions are used */
...
#	define		is_struct(v) ((DbgVerify(!is_sourcepositionwrapper(v)), (v))->tag == INTERNAL_STRUCT_TAG)
	...
	BOOL		is_kind_of(ValueMetaClass* c) override { return (c == class_tag(StructDef)) ? 1 : Value::is_kind_of(c); }
