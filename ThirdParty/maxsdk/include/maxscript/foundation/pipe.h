...
#	define		is_pipe(v) ((DbgVerify(!is_sourcepositionwrapper(v)), (v))->tag == INTERNAL_PIPE_TAG)
	...
	int			at_eos() override;
	unsigned int pos() override; // Gets the current position in the stream
	unsigned int line() override; // Gets the current line number
	void		rewind() override;
	void		flush_to_eol() override;
	void		flush_to_eobuf() override;

	void		put_char(const MaxSDK::Util::Char& c, Value* source_file = nullptr, unsigned int offset = 0, unsigned int line = 1);
	void		put_str(const MCHAR* str, Value* source_file = nullptr, unsigned int  offset = 0, unsigned int line = 1);
	void		put_buf(const MaxSDK::Util::Char* str, UINT count, Value* source_file = nullptr, unsigned int offset = 0, unsigned int line = 1);
