...
#	define is_meshselection(v) ((DbgVerify(!is_sourcepositionwrapper(v)), (v))->tag == class_tag(VertSelectionValue) || (v)->tag == class_tag(FaceSelectionValue) || \
	...
	// utility functions
			int		get_sel_index(const BitArray &vs, int n) const;  // index for n'th item vertex in BitArray
			void	update_sel();
			void	set_sel(BitArray &vs);
			void	sprin1(const MCHAR* type, CharStream* s);
...
public:
	...
	MeshSelection* new_sel(MAXWrapper* own, BYTE stype, int indx = 0) override;
...
#	define		is_vertselection(v) ((DbgVerify(!is_sourcepositionwrapper(v)), (v))->tag == class_tag(VertSelectionValue))
	...
	BitArray	get_sel() override;
	BitArray	get_owner_sel() override { return (owner == nullptr) ? BitArray() : owner->get_vertsel(); }
	void		set_owner_sel(BitArray &sel) override {if (owner != nullptr) owner->set_vertsel(sel); }
	BitArray	get_sel_vertices(Mesh* m) override;
	BitArray	get_sel_vertices(MNMesh* m) override;
	BitArray	get_sel_vertices(PatchMesh* m) override;
	GenericNamedSelSetList& get_named_sel_set_list() override { return owner->get_named_vertsel_set(); }
	int			num_elements(Mesh* m) override { return m->getNumVerts(); } 
	int			num_elements(MNMesh* m) override { return m->VNum(); } 
	int			num_elements(PatchMesh* m) override { return m->getNumVerts(); } 
	BOOL		is_same_selection(Value* s) override { return is_vertselection(s); }
	void		delete_sel(Mesh& m, MeshDelta& md, BitArray &sel) override;
	void		delete_sel(MNMesh* m, ReferenceTarget* owner, BitArray &sel) override; 
...
#	define		is_faceselection(v) ((DbgVerify(!is_sourcepositionwrapper(v)), (v))->tag == class_tag(FaceSelectionValue))
	...
	// specialized utility functions
	BitArray	get_sel() override;
	BitArray	get_owner_sel() override { return (owner == nullptr) ? BitArray() : owner->get_facesel(); }
	void		set_owner_sel(BitArray &sel) override {if (owner != nullptr) owner->set_facesel(sel); }
	BitArray	get_sel_vertices(Mesh* m) override;
	BitArray	get_sel_vertices(MNMesh* m) override;
	BitArray	get_sel_vertices(PatchMesh* m) override;
	GenericNamedSelSetList& get_named_sel_set_list() override { return owner->get_named_facesel_set(); }
	int			num_elements(Mesh* m) override { return m->getNumFaces(); } 
	int			num_elements(MNMesh* m) override { return m->FNum(); } 
	int			num_elements(PatchMesh* m) override { return m->getNumPatches(); } 
	BOOL		is_same_selection(Value* s) override { return is_faceselection(s); }
	void		delete_sel(Mesh& m, MeshDelta& md, BitArray &sel) override;
	void		delete_sel(MNMesh* m, ReferenceTarget* owner, BitArray &sel) override; 
...
#	define		is_edgeselection(v) ((DbgVerify(!is_sourcepositionwrapper(v)), (v))->tag == class_tag(EdgeSelectionValue))
	...
	// specialized utility functions
	BitArray	get_sel() override;
	BitArray	get_owner_sel() override { return (owner == nullptr) ? BitArray() : owner->get_edgesel(); }
	void		set_owner_sel(BitArray &sel) override {if (owner != nullptr) owner->set_edgesel(sel); }
	BitArray	get_sel_vertices(Mesh* m) override;
	BitArray	get_sel_vertices(MNMesh* m) override;
	BitArray	get_sel_vertices(PatchMesh* m) override;
	GenericNamedSelSetList& get_named_sel_set_list() override { return owner->get_named_edgesel_set(); }
	int			num_elements(Mesh* m) override { return m->getNumFaces() * 3; } 
	int			num_elements(MNMesh* m) override { return m->ENum(); } 
	int			num_elements(PatchMesh* m) override { return m->getNumEdges(); } 
	BOOL		is_same_selection(Value* s) override { return is_edgeselection(s); }
	void		delete_sel(Mesh& m, MeshDelta& md, BitArray &sel) override;
	void		delete_sel(MNMesh* m, ReferenceTarget* owner, BitArray &sel) override; 
