...
enum gp_flags2
{
	...
	COLLECTABLE_IS_ANIMATABLE_KNOWN	= 0x0002, // used by gc for caching whether value derives from animatable. Set if cached. See derives_from_Animatable() below.
	COLLECTABLE_IS_ANIMATABLE		= 0x0004, // If bit 2 is set, this bit says whether value derives from animatable or not
...
#define ALLOCATOR_STACK_SIZE_BUFFER 2000	// additional buffer added during allocation to prevent potential overflow when current_result 
														// is in retiring frame and being moved to top of current frame (see pop_alloc_frame())
...
extern ScripterExport void push_alloc_frame(MAXScript_TLS* _tls = nullptr);					// manage alloc stack...
extern ScripterExport void pop_alloc_frame(MAXScript_TLS* _tls = nullptr);
extern ScripterExport void pop_alloc_frame(Value*& result, MAXScript_TLS* _tls = nullptr);	// pops & moves result into callers frame if only on stack
...
#pragma pop_macro("new")
	...
	// The following method tests to see whether the Collectable derives from Animatable. The test is done by attempting to do a 
	// dynamic_cast<Animatable>. Since this is an expensive call we cache the result in 'flags2'. If bit COLLECTABLE_IS_ANIMATABLE_KNOWN
	// is not set, we set it and do the dynamic_cast. If the cast is successful, we set bit COLLECTABLE_IS_ANIMATABLE. If bit 
	// COLLECTABLE_IS_ANIMATABLE_KNOWN is set, we return whether bit COLLECTABLE_IS_ANIMATABLE is set.
	ScripterExport bool derives_from_Animatable(); 
   // following identifies classes that derive from MAXWrapper. Only other implementation is in MAXWrapper
	virtual BOOL	derives_from_MAXWrapper() { return FALSE; }

	ScripterExport void	make_collectable();
				   void	make_permanent();   // no long exported, must use make_heap_permanent AND use its result as the made-permanent value
	...
	// validate collectable_list and permanent_list linkage. Used when debugging of bad values
	ScripterExport static bool validate_value_linkages(); 


private:
	// following no long exported, use new/delete operators
	static void* heap_alloc(size_t sz);
	static void* stack_alloc(size_t sz);
	static void  heap_free(void* p);

	static void post_new(Collectable* c);
...
class ValueMapper : public MaxHeapOperators
{
public:
	virtual ~ValueMapper() {;}
...
{
	if (v != nullptr && Collectable::state != closing_down)
		v->make_collectable();
}

class NewCollectableLinkedListItem : public MaxHeapOperators
{
	friend class Collectable;
	friend void free_thread_locals();
public:
	ScripterExport NewCollectableLinkedListItem();
	ScripterExport NewCollectableLinkedListItem(Collectable* ptr, NewCollectableLinkedListItem* next);
	ScripterExport ~NewCollectableLinkedListItem();

private:
	Collectable* m_ptr;
	NewCollectableLinkedListItem* m_next;

	static ULONGLONG m_live_instances;
};
