...
/*! Class that uses the RAII idiom to push/pop the thread local current_locals_frame.
	...
	an exception. 
	Instances of this class are automatically created via the various XXX_value_local(s) and xxx_typed_value_local(s)
	macros below.
...
/*! Class that uses the RAII idiom to push/pop the thread local current_locals_frame.
	This ensures that the thread local current_locals_frame is always properly restored and memory is freed, even in the event of
	an exception. 
	mpAllocPtr points to memory allocated by _malloca.
	Instances of this class are automatically created via the value_local_array and value_local_array_tls macros below.
*/
class MXS_Exit_ValueLocalArray_Scope : public MaxSDK::Util::Noncopyable
{
	Value** frame;
	MAXScript_TLS* _tls;
	void* mpAllocPtr;
...
public:
	ScripterExport MXS_Exit_ValueLocalArray_Scope(void* alloc_ptr, MAXScript_TLS* tls = nullptr);
	ScripterExport ~MXS_Exit_ValueLocalArray_Scope();
};
...
// Allocate an array on the c++ stack or heap that protects the values held by the array from garbage collection.
// The garbage collection protection is present until the MXS_Exit_ValueLocalArray_Scope instance goes out of scope.
// The MXS_Exit_ValueLocalArray_Scope instance will also free the memory if it was allocated in the heap.
// Example usage:
//	Value** mxsArgs = nullptr;
//	value_local_array(mxsArgs, mxsCount);
...
/*! Class that uses the RAII idiom to allocate and deallocate value temp arrays. 
	...
	an exception. 
*/
class [[nodiscard]] ScopedValueTempArray : public MaxSDK::Util::Noncopyable
{
	Value** m_frame;
	Value** m_localArray;
	MAXScript_TLS* m_tls;
...
*/
class [[nodiscard]] ScopedSaveCurrentFrames : public MaxSDK::Util::Noncopyable
{
public:
...
*/
class [[nodiscard]] ScopedMaxScriptSEHandler : public MaxSDK::Util::Noncopyable
{
...
*/
class [[nodiscard]] ScopedMaxScriptEvaluationContext : public MaxSDK::Util::Noncopyable
{
	bool m_ContextExitted;
	ScopedMaxScriptSEHandler* m_ScopedMaxScriptSEHandler;
	MAXScript_TLS* m_tls;
	Value* m_save_source;
	MSZipPackage* m_save_pkg;
	unsigned int m_save_pos;
	unsigned int m_save_line;
	DWORD m_save_flags;
...
/*! Class that uses the RAII idiom to push/pop the thread local current_controller. 
	...
	an exception. 
*/
class [[nodiscard]] ScopedPushControl : public MaxSDK::Util::Noncopyable
{
	Control* m_save_cc;
	MAXScript_TLS* m_tls;
