...
public:
	...
	const MCHAR* find(const MCHAR* zip_name);
...
public:
	...
	ScripterExport static bool file_in_package(const MCHAR* file_name, MSTR* extract_dir=nullptr);
	...
	const MCHAR*	find_extraction(const MCHAR* zip_name) { return extractions.find(zip_name); }
	void	add_extraction(const MCHAR* zip_name, const MCHAR* extracted_name);

	// directory & file manipulation
	ScripterExport static MSTR	expand_dir(const MSTR& dir);
	ScripterExport static MSTR	expand_file(const MSTR& file_name);
