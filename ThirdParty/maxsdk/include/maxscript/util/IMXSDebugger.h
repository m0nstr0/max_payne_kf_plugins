...
public:
	...
	virtual void OpenDialog(BOOL breakExecution, const MCHAR* outString = nullptr, BOOL setFocus = FALSE) = 0;
	...
	virtual void WriteString(const MCHAR* str) = 0; // output string to output window
	virtual void WriteLine(const MCHAR* str) = 0; // output string to output window, appends \n if needed.

	// Break MAX execution if not already broken. If outString is specified, it is output to the debugger output window
	virtual void BreakExecution(const MCHAR* outString = nullptr) = 0;
