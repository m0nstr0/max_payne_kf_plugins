...
public:
	...
	virtual IVisualMSForm* CreateFormFromFile(const MCHAR* fname)=0;	// create form from a .vms file
...
{
public:
	virtual void			Open(IVisualMSCallback* cb=nullptr, const MCHAR* source=nullptr)=0; // open the form editor on this form
	...
	virtual BOOL			GenScript(MSTR& script, const MCHAR* indent=nullptr)=0;
...
{
public:
	virtual	void	SetPropery(const MCHAR* propName, float f)=0;
	virtual	void	SetPropery(const MCHAR* propName, int i)=0;
	virtual	void	SetPropery(const MCHAR* propName, bool b)=0;
	virtual	void	SetPropery(const MCHAR* propName, const MCHAR* s)=0;
	virtual	void	SetProperyLit(const MCHAR* propName, const MCHAR* s)=0;
	virtual	void	SetPropery(const MCHAR* propName, Point2 pt)=0;
	virtual	void	SetPropery(const MCHAR* propName, Point3 pt)=0;
	virtual	void	SetPropery(const MCHAR* propName, Tab<const MCHAR*>* sa)=0;

	virtual void	SetHandler(const MCHAR* eventName, const MCHAR* source, int arg_start, int arg_end, int body_start, int body_end)=0;
	...
	virtual void    AddComment(const MCHAR* code, int src_from, int src_to)=0;
...
public:
	...
	virtual HWND			GetEditBox() { return nullptr; }

	FPInterfaceDesc*		GetDesc() override { return (FPInterfaceDesc*)GetCOREInterface(VISUALMS_CALLBACK_INTERFACE); }
