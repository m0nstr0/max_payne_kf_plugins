...
with various methods of the class to get/set the UI flags.  kUpdates is used when
you want to temporarily remove a ref from updating the scene graph.  This is for performance
reason for instance if you wanted to cache a bunch nodes for display purposes after you 
built the cache you could turn off this flag and these not will not participate in 
scene updates.
*/
class INodeExposure : public FPMixinInterface {
public:
...
enum {	
		...
		kUpdates, // when off this suspend the node from updating BaseNode::Update, BaseNode::InvalidateTreeTM, 
		          // NodeEventManager, and the NotifyDependents(tmValid, PART_TM, REFMSG_TM_CHANGE, AXIS_DISPLAY_CLASS_ID, FALSE); in Node::UpdateTM
			  // NOTE Node::UpdateTM(TimeValue t) is still processed due to the amount bone specific updates in it (might change)
